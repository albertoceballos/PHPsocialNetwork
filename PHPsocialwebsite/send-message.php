<?php
  ...
  if(isset($_POST['send'])){
    ...
    if(DB::query('SELECT id FROM users WHERE id=:receiver',array(':receiver'=>$_GET['receiver']))){
      DB::query('INSERT INTO messages(body,sender,receiver,isread) VALUES(:body,:sender,:receiver,0)',array(':body'=>$_POST['body'],':sender'=>$userid,':receiver'=>htmlspecialchars($_GET['receiver'])));
      echo 'Message sent';
    }else{
      die('Invalid ID');
    }
